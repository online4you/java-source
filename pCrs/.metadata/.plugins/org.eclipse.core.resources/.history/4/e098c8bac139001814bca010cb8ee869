...
public class HotelanClientStub {
	...
	// XML Test http://www.portvil.com/interface/test.php?us=francisca@online4you.es&pw=bzR5b3U=&agencyId=175
	...
	public HotelanClientStub(){
		...
		try {
			...
			urlBase=endpoint+"?"+"us="+user+"&pw="+password+"&agencyId="+agencia;
		} catch (IOException e) {
	...
	*/
	...
	private String doCall(String urlQueryString, boolean readIntros) throws Exception {
		oLog.error("Ini About to Query:\n"+urlBase+urlQueryString);
	...
	public IrqVillaPreReservationResponse getIrqVillaPreReservation(
		...
		String pet="&request=IrqVillaPreReservation";
		pet=idioma!=null?pet+"&lang="+idioma:pet;
		pet=hotelId!=null?pet+"&villaId="+hotelId:pet;
		pet=arrival!=null?pet+"&arrival="+SystemHelper.getCalendarTo_YYYY_MM_dd(arrival):pet;
		pet=nights!=null?pet+"&nights="+nights:pet;
