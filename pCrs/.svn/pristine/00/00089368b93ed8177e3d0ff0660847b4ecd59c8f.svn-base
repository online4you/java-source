...
if tieneCupo then 'sigo con los otros datos 
	...
	cs="SELECT ISNULL(Traduccion,Texto) AS Tradu FROM " & precrs & "HabitacionesHotel HabitacionesHotel LEFT JOIN " & precrs & "Traducciones Traducciones "
	...
	if hayprecios then
		for prec=0 to ubound(RegPrecios,2)
			...
			if calcular then 
				...
				%><!--#include file="Dto3Persona3.asp"--><%
				%><!--#include file="PrecioRegimen.asp"--><%
				%><!--#include file="DtosRegimen3.asp"--><%
...
for d=1 to noches
	if codiOferta(d)<>"" then 'hay algo
		if codiOferta(d)<>anter then 'otra oferta o la primera
			...
			response.write vbtab & "<textooferta>" & server.HTMLEncode(textoOferta(d)) & "</textooferta>" & vbcrlf
