...
else{alert("WARNING: _editor_url is not set!  You should set this variable to the editor files path;
 it should preferably be an absolute path, like in '/htmlarea/', but it can be relative if you prefer.  Further we will try to load the editor files correctly but we'll probably fail.");
_editor_url='';
} 
...
this.fontname={"&mdash;
 font &mdash;
":'',"Arial":'arial,helvetica,sans-serif',"Courier New":'courier new,courier,monospace',"Georgia":'georgia,times new roman,times,serif',"Tahoma":'tahoma,verdana,arial,helvetica,sans-serif',"Times New Roman":'times new roman,times,serif',"Verdana":'verdana,tahoma,arial,helvetica,sans-serif',"impact":'impact',"WingDings":'wingdings'} 
;
this.fontsize={"&mdash;
 size &mdash;
":"","1 (8 pt)":"1","2 (10 pt)":"2","3 (12 pt)":"3","4 (14 pt)":"4","5 (18 pt)":"5","6 (24 pt)":"6","7 (36 pt)":"7"} 
;
this.formatblock={"&mdash;
 format &mdash;
":"","Heading 1":"h1","Heading 2":"h2","Heading 3":"h3","Heading 4":"h4","Heading 5":"h5","Heading 6":"h6","Normal":"p","Address":"address","Formatted":"pre"} 
;
this.customSelects={} 
;
...
switch(typeof id){case "string":return this.btnList[id]=[tooltip,image,textMode,action,context];
case "object":return this.btnList[id.id]=[id.tooltip,id.image,id.textMode,id.action,id.context];
} 
} 
;
HTMLArea.Config.prototype.registerDropdown=function(object){if(typeof this.customSelects[object.id]!="undefined"){} 
...
iframe.src=HTMLArea.is_ie?"javascript:false;
":"about:blank";
...
editor._doc=doc;
if(!editor.config.fullPage){doc.open();
...
html+="<style type='text/css'>"+editor.config.pageStyle+" html,body { border: 0px;
 } 
</style>\n";
html+="</head>\n";
...
;
function clearStyle(node){var declarations=node.style.cssText.split(/\s*;
\s*/),line;
...
;
HTMLArea.prototype.focusEditor=function(){switch(this._editMode){case "wysiwyg":try{this._iframe.contentWindow.focus()} 
catch(e){} 
break;
case "textmode":try{this._textArea.focus()} 
catch(e){} 
break;
...
info+=this.el.style.cssText.split(/;
\s*/).join(";
\n");
alert(info);
return false;
...
if(this._customUndo&&!this._timerUndo){this._undoTakeSnapshot();
var editor=this;
...
HTMLArea.prototype.dom_checkInsertP=function(step2){var i,df,tmp,p=this.getAllAncestors(),block=null,pnodes={} 
...
;
HTMLArea.htmlEncode=function(str){return str.replace(/&/ig,"&amp;
").replace(/</ig,"&lt;
").replace(/>/ig,"&gt;
").replace(/\x22/ig,"&quot;
").replace(/\u00A0/g,"&#xa0;
...
;
HTMLArea.onBeforeUnload=function(){return "Leaving this page will dismiss any changes you might have done to this document.  "+"Please confirm that you want to throw away the changes.";
} 
;
