...
if laid<>0 then 'Busco el registro para modificar
	cs="SELECT Categorias.Id,Nombre,IdTipo,Idioma,Traduccion FROM " & precrs & "Categorias Categorias LEFT JOIN " & precrs & "Traducciones Traducciones "
	cs=cs & "ON Categorias.Id=Traducciones.IdReferencia "
	cs=cs & "AND Tabla='Categorias' AND Campo='Nombre' WHERE Categorias.Id=" & laid
	rs.open cs,base
	do while not rs.eof
		TIdiomas(0)=rs("nombre")
		idtipo=paClng(rs("idtipo"))
...
<div class="tituloFicha" id='tituloFicha'>
	<div class="nombreFicha"><%=objIdioma.getTraduccionHTML("i_categoria")%> -> <%=TIdiomas(0)%></div>
