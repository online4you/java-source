CREATE TABLE Categorias (Id int IDENTITY(1,1) NOT NULL PRIMARY KEY,Nombre nvarchar(50),IdTipo int default(0));
...
Categoria int DEFAULT(0),NombrePagina nvarchar(25),Foto nvarchar(100), Visitas int default(0));
...
Observaciones nvarchar(MAX) NULL,NombrePais nvarchar(50),CodigoVIP nvarchar(25),
Tratamiento int DEFAULT(0),Activo bit DEFAULT(0), Usuario nvarchar(50), Clave nvarchar(50));

CREATE INDEX FICHAS_CODRESERVA ON FICHAS (Id);
CREATE INDEX FICHAS_VIP ON FICHAS (CodigoVIP);

CREATE TABLE Tratamientos (Id int IDENTITY (1,1) NOT NULL PRIMARY KEY,
...
Texto nvarchar(MAX) NULL,FechaInicio Datetime,FechaFin Datetime,Valida datetime,Caduca Datetime,
...
NochesGratis int DEFAULT(0),CodigoPromocion nvarchar(25),DiasAdelanto int DEFAULT(0),Tarifa int default(1));
...
FechaReserva Datetime,HoraLlegada nvarchar(50),NumDias int DEFAULT(0),Servicios nvarchar(50),
...
CodOferta nvarchar(100),PelasOferta nvarchar(100),IdCliente int DEFAULT(0),
...
Minimo int DEFAULT(1), Prepago real DEFAULT(0));
...
CunaOcupa bit DEFAULT(0), AdmiteNinos bit default(1));
...
PrePreBase real DEFAULT(0),PrePerHab bit DEFAULT(0),Anyo int DEFAULT(0),Tarifa int default(1));
...
Zona nvarchar(50),PosTop float DEFAULT(0),PosLeft float DEFAULT(0),
Zoom int DEFAULT(0),Foto nvarchar(50),IdPadre int default(0));
...
CREATE TABLE Tarifas (Id int IDENTITY(1,1) NOT NULL PRIMARY KEY, Nombre nvarchar(50));
...
CREATE TABLE GrupoCaracteristicas (Id int IDENTITY(1,1) NOT NULL PRIMARY KEY, Nombre nvarchar(100), Orden int default(0), Destacada bit default(0));

CREATE TABLE Caracteristicas (Id int IDENTITY(1,1) NOT NULL PRIMARY KEY, Caracteristica nvarchar(100), Icono nvarchar(75), Orden int default(0), Destacada bit default(0), IdGrupo int default(0));
CREATE INDEX IndiCaracterG ON Caracteristicas (IdGrupo);
...
CREATE TABLE FPagoHotel (Id int IDENTITY(1,1) NOT NULL PRIMARY KEY, CodigoEsta int DEFAULT(0), 
tipoFPago int default(0), CodComercio nvarchar(50), Terminal nvarchar(50), Clave nvarchar(100),
claveXor nvarchar(100),Produccion bit default(0),AcquirerBIN nvarchar(25));
CREATE INDEX IndiFPHotel ON FPagoHotel (CodigoEsta);


CREATE TABLE Menus (Id int IDENTITY(1,1) NOT NULL PRIMARY KEY, Titulo nvarchar(75), 
IdPadre int DEFAULT(0), TipoEnlace int DEFAULT(0), TipoOpen int DEFAULT(0), URL nvarchar(200),
Ancho int DEFAULT(0), Alto int DEFAULT(0), Scroll bit DEFAULT(0), FechaInicio DATETIME,
FechaCaduca DATETIME, Orden int DEFAULT(0), NivelAcceso int DEFAULT(0), Activo bit DEFAULT(0));
CREATE INDEX IndiMenu ON Menus (IdPadre);

CREATE TABLE Contenidos (Id int IDENTITY(1,1) NOT NULL PRIMARY KEY, Titulo nvarchar(200),
IdPadre int DEFAULT(0),TextoBreve nvarchar(MAX),TextoCompleto nvarchar(MAX),
Fecha DATETIME, FechaCaduca DATETIME, Orden int DEFAULT(0), NivelAcceso int DEFAULT(0), Activo bit DEFAULT(0));
CREATE INDEX IndiConte ON Contenidos (IdPadre);

CREATE TABLE FotosContenido (Id int IDENTITY(1,1) NOT NULL PRIMARY KEY,
IdContenido int DEFAULT(0),Foto nvarchar(100) NULL, Orden int DEFAULT(0));
CREATE INDEX FConte ON FotosContenido (IdContenido);

CREATE TABLE EstadisticaHotel (Id int IDENTITY(1,1) NOT NULL PRIMARY KEY,
Fecha DATETIME default(getdate()),IP nvarchar(25),CodigoEsta int default(0),lang nvarchar(2));
CREATE INDEX VHotel ON EstadisticaHotel (CodigoEsta);

CREATE VIEW ConsultaHoteles AS SELECT Establecimientos.CodigoEsta,Establecimientos.Nombre, Establecimientos.Estado, Establecimientos.Moneda, DatosHotel.Poblacion, DatosHotel.Zona, DatosHotel.TipoAlojamiento, DatosHotel.Categoria, Establecimientos.Orde FROM Establecimientos INNER JOIN DatosHotel ON Establecimientos.CodigoEsta = DatosHotel.CodigoEsta;
...
CREATE FUNCTION [dbo].[fnGetTraduccion]
(
	@Tabla nvarchar(75),
	@Campo nvarchar(75),
	@IdReferencia int,
...
BEGIN
	...
	DECLARE @Traduccion nvarchar(MAX)
	
	SELECT @Traduccion=Traduccion FROM Traducciones
	WHERE Tabla=@Tabla AND Campo=@Campo AND Idioma=@Idioma AND IdReferencia=@IdReferencia

	RETURN @Traduccion

END;

CREATE PROCEDURE [dbo].[visitaHotel] (
	@ip char(25),
	@hotel int,
	@lang char(2)
)
AS
BEGIN
    UPDATE DatosHotel SET Visitas=ISNULL(Visitas,0)+1 WHERE CodigoEsta=@hotel
	
	INSERT INTO EstadisticaHotel (IP,CodigoEsta,Lang) 
	VALUES (@ip,@hotel,@lang)
END
