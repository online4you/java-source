...
if cpromo<>"" then 'buscar titulo oferta
	cs="SELECT Id,IF(ISNULL(traduc.Traduccion),Ofertas.Titulo,traduc.Traduccion) Titulo "
...
if request.cookies("idAgencia")="" then 'comprobar
	...
	if idagencia<>0 then 'buscar datos
		cs="SELECT Nombre FROM " & precrs & "Agencias WHERE Id=" & idagencia
		rs.open cs,base
		if not rs.eof then
...
<div id='principalFrame'> 
  ...
  <div id='frameCabecera'> 
    <form name="fb" method="post" action="<%=MiPag%>?ide=<%=idEmpresa%>&amp;lang=<%=lang%>&amp;pr=paso1">
      ...
      <%=objIdioma.getTraduccionHTML("i_fsalida") & ": "%> <a id='affin' href="javascript:abreCalendarFrame('ffin');"><%=ffin%></a> 
      ...
      <div class="texto<%=displayPromo%>"> <%=objIdioma.getTraduccionHTML("i_codpromocion") & ": "%> 
        <input type="text" name="cpromo" id='cpromo' value="<%=cpromo%>" />
        <span id='textopromo'></span> 
  ...
  <div id='contenidoFrame'> 
    ...
    <div class="resultado"> 
      ...
      <div class="izq_resultado"> <a href="http://booking.kubikcrs.com/fichaHotel.asp?ide=<%=idEmpresa%>&amp;idh=<%=Lista(LCodi,h)%>&amp;lang=<%=lang%>"> 
      ...
      <div class="der_resultado"> 
        ...
        <div class="textoHotel"> <%=Lista(LDescri,h)%> </div>
        <!--textohotel-->
      </div>
      <!--der_resultado-->
      <div class='habisHotel'> 
        <%if idEmpresa = 94 then%>
        <div class="col_habi"> <%=objIdioma.getTraduccionHTML("i_diasminimos")%>&nbsp;<%=Lista(LHDiasMin,h)%> 
        </div>
        <div class="clear"></div>
        <%end if%>
        <%
				...
				%>
        <div class="col_habi"> <span class="iconoHabi">&nbsp;</span>&nbsp;<%=Lista(LHabi,t)%>&nbsp; 
        ...
        <!--regimen-->
        <a id='enlace_<%=Lista(LCodi,t)%>_<%=Lista(LCHabi,t)%>' href="paso2.asp?ide=<%=idempresa%>&amp;idh=<%=Lista(LCodi,t)%>&amp;lang=<%=lang%>&amp;fini=<%=fini%>&amp;ffin=<%=ffin%>&amp;ad=<%=adultos%>&amp;ni=<%=ninos%>&amp;th=<%=Lista(LCHabi,t)%>&amp;tem=<%= temporada %>" class="col_reserva"><%=objIdioma.getTraduccionHTML("i_reservar")%></a> 
        <a id='precio_<%=Lista(LCodi,t)%>_<%=Lista(LCHabi,t)%>' href="paso2.asp?ide=<%=idempresa%>&amp;idh=<%=Lista(LCodi,t)%>&amp;lang=<%=lang%>&amp;fini=<%=fini%>&amp;ffin=<%=ffin%>&amp;ad=<%=adultos%>&amp;ni=<%=ninos%>&amp;th=<%=Lista(LCHabi,t)%>&amp;tem=<%= temporada %>" class="col_precio"> 
        <b><%=formatnumber(pelas * elCambio, 2) & sufijoMoneda %></b></a> <img id='espera_<%= Lista(LCodi,t) %>_<%= Lista(LCHabi,t) %>' src="img/espera.gif" width="16" height="16" class="floatRight" /> 
        <br class='clear' />
        <%if idEmpresa=94 then%>
