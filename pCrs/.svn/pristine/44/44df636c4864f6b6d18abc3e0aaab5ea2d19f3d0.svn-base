...
%>
<!--#include file="ActuFotosHabitacion.asp"-->
<%

cs="SELECT Id,Foto,Orden FROM " & precrs & "FotosHabitacion WHERE IdHabitacion=" & laid
...
function Modificar(esa){
	if (compruebaFoto(document.getElementById('foto_'+esa))) {
		...
		if (esa==0)
			...
			document.f1.action="<%=MiPag%>?modo=actu&idh=<%=laid%>&idf="+esa;
		document.f1.submit();
	}
}
...
function cambiaOrden(esa) {
	...
	url="actuOrdenFotos.asp?tabla=FotosHabitacion&idf="+idfoto+"&ord="+esa.value+"&recarga="+recarga;
	//alert(url);
...
<div style="height:480px; width:970px; overflow:hidden;">		
