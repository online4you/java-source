...
redim OfertaHabPct(noches)
redim OfertaRegimen(noches)
redim OfertaRegimenPct(noches)
...
'response.write(tieneCupo & " tieneCupo******************<br>")
if tieneCupo then 'sigo con los otros datos 
	...
	precioValido=true
	'response.write(hayprecios & " hayprecios******************<br>")
	if hayprecios then
...
for d = 1 to noches
	...
	o = 0
	o = o + OfertaHabPct(d)
	o = o + OfertaRegimenPct(d)

	if d <> 1 then
		ofertasPct = ofertasPct & ";"
	end if

	ofertasPct = ofertasPct & o	
next
