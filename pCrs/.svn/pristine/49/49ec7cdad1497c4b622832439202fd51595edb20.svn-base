...
pasalir=0
cs="WHERE Fichas.CodigoVIP<>'' AND Activa<>0 "
if request.form<>"" then 'hacer filtro
	bcodigo=request.form("bcodigo")
	if bcodigo<>"" then cs=cs & "AND Fichas.CodigoVIP='" & bcodigo & "' "
	...
	if isdate(altadesde) and isdate(altahasta) then cs=cs & "AND (FechaAlta BEETWEEN " & FechaMySQL(altadesde) & " AND " & FechaMySQL(altahasta) & ") "
	
	bnombre=request.Form("bnombre")
	if bnombre<>"" then cs=cs & "AND (Fichas.Nombre LIKE '%" & ControlAcentos(bnombre) & "%' OR Apellidos LIKE '%" & ControlAcentos(bnombre) & "%') "
	
	response.Cookies("cookieFiltroVisita")=cs
	pasalir=1
end if
...
<%if pasalir=1 then%>
	...
	top.frames['elFrame<%=ff%>'].location="visitas.asp?p=<%=pag%>";
	cerrar();
<%end if%>

function SinFiltro(){
	top.frames['elFrame<%=ff%>'].location="visitas.asp?sf=si&recarga="+self.name;
	cerrar();
}
</script>
<body class="laFicha">
    ...
    <td class="tdTabla">
		<table align='center' width='400' border='0' cellpadding="0" cellspacing="0" style="margin-top:10px;">
			...
			<tr>
				...
				<td align='left'>Desde <input type='text' style='width:80px' maxlength='15' name='reservadesde'>
				&nbsp;&nbsp;&nbsp;&nbsp;hasta <input type='text' style='width:80px' maxlength='15' name='reservahasta'>
				</td>
			</tr>
			<tr>
				<td align='right'>Fecha Estancia:</td>
				<td align='left'>Desde <input type='text' style='width:80px' maxlength='15' name='estandesde'>
				&nbsp;&nbsp;&nbsp;&nbsp;hasta <input type='text' style='width:80px' maxlength='15' name='estanhasta'>
				</td>
			</tr>
			<tr>
