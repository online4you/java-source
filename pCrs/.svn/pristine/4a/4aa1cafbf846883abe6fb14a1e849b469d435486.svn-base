<%
	...
	'response.Write(adultos & "<br>")
	'response.Write(ninos & "<br>")
	'response.Write(idh & "<br>")
	'response.Write(ninos & "<br>")
	...
	'Valores del array
	dimensionArray=18 'se usa para poder redimensionar el array a 19 si es BCM para poder recoger un parámetro más de días mínimos
	nlista=-1
	LCodi=0
	LNombre=1
	LTipo=2
	...
	if objDom.Load(xmlURL & parametros) then
		...
		For Each objItem in objDom.documentElement.SelectNodes("/data/hotel")
			...
			Lista(LTemporada,nlista)=objItem.SelectSingleNode("temporada").text
			...
			For Each eso in objItem.SelectNodes("habitacion")
				if primera then 'primera hab
					...
					nlista=nlista+1
					redim preserve Lista(dimensionArray, nlista)
