...
if lang="" then lang=request.querystring("lang")
if lang="" then lang=langPorDefecto 'definida en constantes del cliente
lang=lcase(lang) 'me gusta mas en minúsculas
'Comprobar que lang esté en la lista de idiomas
esta=false
for i=0 to ubound(ListaIdiomas)
	if listaIdiomas(i)=lang then
		esta=true
		exit for
	end if
next
if not esta then lang=listaIdiomas(0) 'por defecto
...
class clsIdioma
	...
	Private Sub Class_Initialize()
		xmlFichero=Server.MapPath(rutaXMLIdioma & "idioma_"+lang+".xml")
	...
	public Sub cargaIdioma(este)
		set xmlFichero=nothing 'por si hay uno abierto
		xmlFichero=Server.MapPath(rutaXMLIdioma & "idioma_"+este+".xml")
		'response.write xmlFichero
		Set idiomaXML = Server.CreateObject("Microsoft.XMLDOM")
		idiomaXML.async=false
		if not idiomaXML.Load(xmlFichero) then 'no existe
