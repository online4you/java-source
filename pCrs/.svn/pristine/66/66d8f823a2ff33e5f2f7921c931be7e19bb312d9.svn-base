...
<div id='principalFrame'>
    ...
    <div id='contenidoFrame'>
		...
		<div id='fotosHotel'>
        	<img id='fotoPrincipal' src="<%=renombraFoto(fotoHotel,"")%>" width="274" border="0" alt="<%=nombreHotel%>" />
					...
					for ff=0 to ubound(lfotos)
						if trim(lfotos(ff))<>"" then
							...
							<div class='coluFoto'>
								...
								<a href="<%=renombraFoto(fotoHotel,"")%>" class="thumbnail">
							...
							<%end if 'nfotos=0							
							nfotos=nfotos+1%>
							<a href="<%=renombraFoto(lfotos(ff),"")%>" class="thumbnail">
    ...
    <%if hayhabis then%>
	    <div class="contenidoHotel">
            ...
            <div class="habiHotel">
                ...
                <div class="fotosHabi">
                	<img id='fhabi_<%=h%>' src="<%=renombraFoto(lfotos(0),"")%>" class='fotoHabitacion' alt="<%=nombreHotel%>" />
                   	<p class="ahabi">
                    <%for f=0 to ubound(lfotos)
					if trim(lfotos(f))<>"" then%>
                    <a id='fhabi_<%=h%>-<%=f%>' href="<%=renombraFoto(lfotos(f),"")%>" class="thumbnailHabi"><%=f+1%></a>
                    <%end if
					next 'f%>
                    </p>
                </div>   
                ...
                <a href="https://booking.kubikcrs.com/paso2.asp?ide=<%=idempresa%>&amp;idh=<%=idh%>&amp;lang=<%=lang%>&amp;th=<%=RegHabis(HaCodi,h)%>" class="col_reserva"><%=objIdioma.getTraduccionHTML("i_reservar")%></a>
            </div> <!-- habihotel-->
