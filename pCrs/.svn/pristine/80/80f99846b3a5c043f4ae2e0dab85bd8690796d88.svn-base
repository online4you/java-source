...
%><!--#include file="actuCaracter.asp"--><%

redim TIdiomas(ubound(ListaIdiomas))
laid=paClng(request.QueryString("id"))
if laid<>0 then 'Busco el registro para modificar
	cs="SELECT Caracteristicas.Id,Caracteristica,Orden,Icono,Destacada,IdGrupo,Idioma,Traduccion "
	cs=cs & "FROM " & precrs & "Caracteristicas Caracteristicas LEFT JOIN " & precrs & "Traducciones Traducciones "
	cs=cs & "ON Caracteristicas.Id=Traducciones.IdReferencia "
	cs=cs & "AND Tabla='Caracteristicas' AND Campo='Caracteristica' WHERE Caracteristicas.Id=" & laid
	rs.open cs,base
	if not rs.eof then
		do while not rs.eof
			TIdiomas(0)=rs("caracteristica")
			orden=rs("orden")
			icono=rs("icono")
			destacada=rs("destacada")
			IdGrupo=paClng(rs("IdGrupo"))
...
'tabla grupos
cs="SELECT Id,Nombre "
cs=cs & "FROM " & precrs & "GrupoCaracteristicas ORDER BY Orden,Id"
rs.open cs,base
haygrupos=false
if not rs.eof then
	RegGrupos=rs.getrows
	GCodi=0
	GNombre=1
	haygrupos=true
end if
...
<div class="tituloFicha" id='tituloFicha'>
	<div class="nombreFicha"><%=objIdioma.getTraduccionHTML("i_caracter")%> -> <%=TIdiomas(0)%></div>
...
<table border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <%next 'idi%>
    <tr><td align='right'><%=objIdioma.getTraduccionHTML("i_grupocaracter")%>:</td>
		<td align='left'>
        <select name="idgrupo">
        <%if haygrupos then
		for g=0 to ubound(RegGrupos,2)
			marca=""
			if RegGrupos(GCodi,g)=IdGrupo then marca=" selected"%>
            <option value="<%=RegGrupos(GCodi,g)%>"<%=marca%>><%=RegGrupos(GNombre,g)%></option>
        <%next
		end if 'haygrupos%>
        </select>
        </td>
	</tr>
    <tr><td align='right'><%=objIdioma.getTraduccionHTML("i_icono")%>:</td>
		<td align='left' valign="middle">
        <%if icono<>"" then 'poner icono%>
		<img src="<%=rutaFotos & icono%>" align="left" style="max-width:40px;">
		<%end if 'icono<>""%>
        &nbsp;<input type="file" name="icono" id='icono' width="30">
        </td>
	</tr>
