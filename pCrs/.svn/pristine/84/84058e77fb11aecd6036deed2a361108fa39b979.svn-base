...
obs=quitarApos(request.form("com2"))
...
parametros=parametros & "&com=" & server.URLEncode(obs)
parametros=parametros & "&checkFactura=" & checkFactura
parametros=parametros & "&factNombre=" & factNombre
...
parametros=parametros & "&telContact=" & server.URLEncode(telContact)
...
parametros=parametros & "&typeOfPaymentID=" & typeOfPaymentID
...
for h=1 to ubound(Vhabis)
	...
	parametros=parametros & "&prepago-" & h & "=" & prepagoHab(h)
next

responselog "<!--" & xmlURL & parametros & "-->"
