...
cs="SELECT FechaReserva,Cod_Res,Activa,Importe "
...
cs=cs & "GROUP BY FechaReserva,Cod_Res,Activa,Importe ORDER BY FechaReserva"
hayestemes=false
rs.open cs,base
if not rs.eof then
	RegMes=rs.getrows
		...
		for r=0 to ubound(RegMes,2)
			if month(RegMes(MFecha,r))=d then 'este es este mes
				if RegMes(MActi,r) then 'Activa
					diatrue(d)=diatrue(d)+RegMes(MPelas,r)
				else
					diafalse(d)=diafalse(d)+RegMes(MPelas,r)
				end if
			end if
		next	
	next
...
<div id='iframePrincipal'>
  ...
  <div id='iframeConte'> 
	...
	%>
              <tr> 
                <td align='center'> <a href="<%=MiPag%>?est=<%=est & busca%>&todos=<%=todos%>"><%=objIdioma.getTraduccionHTML("i_anyoanterior")%></a> 
                  <b> 
                  <%if todos="-1" then response.write "(" & objIdioma.getTraduccionHTML("i_todosloshoteles") & ") "%>
                  <%=objIdioma.getTraduccionHTML("i_importesrealizados")%>&nbsp;<%=any%></b> 
                  <%
		busca="&any=" & (any+1)
	%>
                  <a href="<%=MiPag%>?est=<%=est & busca%>&todos=<%=todos%>"><%=objIdioma.getTraduccionHTML("i_anyosiguiente")%></a> 
                </td>
              </tr>
              <tr>
              ...
              <tr>
				...
				for I=1 to 12
					...
					response.write "<a href='EstadisticaMesEuros.asp?est=" & est & "&elmes=" & i & "&any=" & any & "&todos=" & todos & "' style='text-decoration:underline'>"
              ...
              <tr>
                <td align="center"> <br/> <input type='button' class="boton145" value='<%=objIdioma.getTraduccionHTML("i_estadisticasanyo")%>' onclick="javascript:window.location='EstadisticaAnyEuros.asp?est=<%=est%>&any=<%=any%>&todos=<%=todos%>';"> 
                  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
