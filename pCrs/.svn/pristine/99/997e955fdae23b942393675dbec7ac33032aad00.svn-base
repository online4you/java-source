...
if hayhabis then
	for h=0 to ubound(RegHabis,2)
		...
		response.Write "fotohabi[" & RegHabis(HaCodi,h)  & "] ='" & replace(RegHabis(HaFotos,h),"http://admin.kubikcrs.com/fotos","https://booking.kubikcrs.com/fotos") & "';" & vbcrlf
...
function verifica(){
	...
	if (guena){
		...
		document.f1.action='https://booking.kubikcrs.com/paso3.asp?ide=<%=IdEmpresa%>&idh=<%=idh%>&lang=<%=lang%>';
		document.f1.submit();
	}
}
