...
function FechaHoraSQLServer(esafecha)
	if request.Cookies("mysqlBD")="1" then
...
Sub controlRegistro(elproceso)
	idusu=clng("0" & request.Cookies("idCR")) 'id del usuario
	idnom=request.Cookies("userCR") 'Nombre del usuario
	codhotel=est
	if codhotel="" then codhotel=0
	anyotempo=anyo
	if anyotempo="" then anyotempo=0
	ccs="INSERT INTO Registro (IdUsuario,Nombre,Fecha,CodigoEsta,Anyo,Proceso) VALUES ("
