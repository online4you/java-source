...
<!--#include file="CR_cargaRegimenes.asp"-->
<%
...
function recalculo(ese,nombreRegi,hotel,habi) {
	...
	$('#paProcesos').attr("src","Frame_recalculo.asp?ide=<%=idEmpresa%>&est="+hotel+"&fini=<%=fini%>&ffin=<%=ffin%>&th="+habi+"&tr="+ese+"&ad=<%=adultos%>&ni1=<%=ninos%>&promo=<%=cpromo%>&moneda=<%=coin%>&lang=<%=lang%>");	
}
</script>
</head>
<body>
	...
	for h=0 to ubound(Lista,2)%>
		<div class="resultado">
			...
			<div class='habisHotel'>
                ...
                <div id='regi_<%=Lista(LCodi,h)%>_<%=Lista(LCHabi,h)%>' class='capa_lista'>
                    <span id='titulo_<%=Lista(LCodi,h)%>_<%=Lista(LCHabi,h)%>' class='titulo_lista'>
                    <%=Lista(LRegi,h)%></span>
                    <div id='listaHabi_<%=Lista(LCodi,h)%>_<%=Lista(LCHabi,h)%>' class="lista">
                    <%if haySuples then
                    for ss=0 to ubound(RegSuples,2)
                        if RegSuples(SHotel,ss)=paClng(Lista(LCodi,h)) then 'es de este hotel
                        if RegSuples(SHabi,ss)=0 or RegSuples(SHabi,ss)=paClng(Lista(LCHabi,h)) then%>
                        <a href="javascript:recalculo(<%=RegSuples(SIdRegi,ss)%>,'<%=RegSuples(SNombre,ss)%>',<%=Lista(LCodi,h)%>,<%=Lista(LCHabi,h)%>);">
                ...
                <a id='enlace_<%=Lista(LCodi,h)%>_<%=Lista(LCHabi,h)%>' href="paso2.asp?ide=<%=idempresa%>&amp;idh=<%=Lista(LCodi,h)%>&amp;lang=<%=lang%>&amp;fini=<%=fini%>&amp;ffin=<%=ffin%>&amp;ad=<%=adultos%>&amp;ni=<%=ninos%>&amp;th=<%=Lista(LCHabi,h)%>" class="col_reserva"><%=objIdioma.getTraduccionHTML("i_reservar")%></a>
                <a id='precio_<%=Lista(LCodi,h)%>_<%=Lista(LCHabi,h)%>' href="paso2.asp?ide=<%=idempresa%>&amp;idh=<%=Lista(LCodi,h)%>&amp;lang=<%=lang%>&amp;fini=<%=fini%>&amp;ffin=<%=ffin%>&amp;ad=<%=adultos%>&amp;ni=<%=ninos%>&amp;th=<%=Lista(LCHabi,h)%>" class="col_precio">
                <b><%=formatnumber(pelas*elCambio,2) & sufijoMoneda%></b></a>
                <img id='espera_<%=Lista(LCodi,h)%>_<%=Lista(LCHabi,h)%>' src="img/espera.gif" width="16" height="16" class="floatRight" />
                <br class='clear' />
