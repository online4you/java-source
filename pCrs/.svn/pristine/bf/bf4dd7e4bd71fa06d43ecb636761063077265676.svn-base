...
if request.Form<>"" then
	...
	cs="SELECT Id FROM " & precrs & "DescuentosColectivos DescuentosColectivos WHERE CodigoColec=" & colec & " AND Temporada=" & tempo
	...
	if rs.eof then
		...
		cs=cs & "DesdePlazas,HastaPlazas) VALUES ("
		cs=cs & colec & "," & tempo & "," & laId & "," & quitarComa(dto) & "," & quitarComa(precio)
		cs=cs & "," & anyo & "," & laTarifa & "," & desdeplazas & "," & hastaplazas & ")"
		base.execute cs
...
<table align='center' width='100%' border='0' cellpadding="0" cellspacing="2" class="tdTabla">
	...
	<%end if%>
    <tr><td colspan="2"><%=objIdioma.getTraduccionHTML("i_textodto3")%></td></tr>
    ...
    <tr><td align="right"><%=objIdioma.getTraduccionHTML("i_temporada")%></td>
    	<td>
