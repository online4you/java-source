...
accomodationtypeids=request.querystring("accomodationtypeids")
...
if errormsg="" then 'sigo buscando
	...
	if accomodationtypeids<>"" and accomodationtypeids<>"0" then 
		lservi=replace(accomodationtypeids,"-",", ")
		cs=cs & " AND DatosHotel.TipoAlojamiento IN ( " & lservi & ") "
	end if 'accomodationtypeids
	cs=cs & " ORDER BY "
	If(orden = "price") Then
						...
						if haytipoa then
							for t=0 to ubound(RegTipoA,2)
								if RegTipoA(RTCodi,t)=RegHotel(HoTipoa,h) then
									response.write vbtab & "<nombreTipo>" & server.HTMLEncode(RegTipoA(RTNombre,t)) & "</nombreTipo>" & vbcrlf
									Exit For
								end if
							next 't
						end if 'haytipoa
