...
'cupo y estado
cs="SELECT Dia,CodigoHab,Cupo,Estado,SUM(Cuantas) as Disponibles "
...
cs=cs & ") GROUP BY CodigoHab,Dia,Cupo,Estado "
...
<div class="tituloFicha" id='tituloFicha'>
	<div class="nombreFicha">DISPONIBILIDAD / ESTADO HABITACIÓN  -> <%=NHotel%></div>
	<div class="laX" id='laX'></div>
	<img id='iconoForma' src="img/Mini.gif" border="0" width="21" height="17" class="Minimizar" alt="Minimizar" />
</div>
<form name="f1" method="post" action="<%=MiPag%>?est=<%=est%>&mes=<%=mes%>&anyo=<%=el_anyo%>&recarga=<%=recarga%>">
...
<table id='losmeses' align="center" width="100%" cellpadding="0" cellspacing="0" style="margin-top:3px;">
	...
	for mm=0 to 2 'pa sumar los meses
		...
		<%for d=1 to 31
			...
			if d=31 then response.write "<td width='15' style='border:none'></td></tr>"
		next%>
				...
				if isdate(midia) then
					...
					for hh=0 to ubound(RegLista,2)
						if RegLista(PDia,hh)=cdate(midia) AND RegLista(PHabi,hh)=RegHabis(HCodi,h) then 'este es el día
							haydia=true
							cupo=RegLista(PCupo,hh)
							testado=RegLista(PEstado,hh)
							ocu=paClng(RegLista(POcu,hh))
							exit for
						end if
					next 'hh
					if haydia then
						response.write "<td align='center'" & fondillo & colorEstado(testado) & " id='" & right("0"&day(midia),2) & "/" & right("0"&month(midia),2) & "/" & year(midia) & "_" & RegLista(PHabi,hh) & "' onclick='javascript:marcar(this);' class='celdilla'>" & cupo & "</td>" & vbcrlf
					else
						response.write "<td align='center'" & fondillo & " style='width:22px'> </td>" & vbcrlf
					end if
				else
					response.write "<td></td>" & vbcrlf
				end if
