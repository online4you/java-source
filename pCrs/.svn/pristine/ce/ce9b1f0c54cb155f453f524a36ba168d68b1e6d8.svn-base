...
if errormsg="" then 'sigo buscando
	...
	'ConsultaHoteles.Estado != 0
'AND (DatosHotel.Zona = 1 
'OR Zonas.IdPadre = 1)
	...
	if Tzona<>0 then busco=busco & "(DatosHotel.Zona = " & cstr(tzona) & " OR Zonas.IdPadre = " + cstr(tzona) + ") AND "
	...
	cs = cs & "ConsultaHoteles.TipoAlojamiento, ConsultaHoteles.Estado, DatosHotel.Foto, ConsultaHoteles.Moneda, Zonas.IdPadre"
	...
	cs=cs & " FROM (ConsultaHoteles INNER JOIN (DatosHotel INNER JOIN Zonas ON DatosHotel.Zona = Zonas.Id) ON ConsultaHoteles.CodigoEsta = DatosHotel.CodigoEsta) "
	...
	if tservicio<>"" then 'busca por caracteristicas
		cs =cs & "LEFT JOIN CaracteristicasHotel "
		cs =cs & "ON ConsultaHoteles.CodigoEsta=CaracteristicasHotel.CodigoEsta "
	end if 'tservicio<>""
	if ide = 94 then
		cs = cs & " inner join Cupos on ConsultaHoteles.CodigoEsta = Cupos.CodigoEsta "
	end if
	cs=cs & busco
	if ide = 94 then
