	...
	select case modo 
		case "borra" 'Borrar marcadas
			...
			if ubound(queborro)>=0 then
				cs="DELETE FROM " & precrs & "OfertasVIP WHERE "
...
'Lista de registros
cs="SELECT OfertasVIP.Id,ISNULL(Traduccion,Titulo) AS Tradu,FechaInicio,FechaFin,APlicarEn,Dto,Caduca,Calcula,Activa "
cs=cs & "FROM " & precrs & "OfertasVIP OfertasVIP LEFT JOIN " & precrs & "Traducciones Traducciones "
cs=cs & "ON OfertasVIP.Id=Traducciones.IdReferencia AND Tabla='OfertasVIP' AND Campo='Titulo' "
...
function enBlanco(){
	...
	top.creaFlotante("verOfertasVIP.asp?id=0&est=<%=est%>&recarga="+self.name,820,465,0,0);
}
function verFicha(id){
	//palIframe(document.getElementById("verFicha"),800,410,0,0,"verOfertas.asp?id="+id+"&est=<%=est%>");
	top.creaFlotante("verOfertasVIP.asp?id="+id+"&est=<%=est%>&recarga="+self.name,820,465,0,0);
}
</script>
</head>
<body>
