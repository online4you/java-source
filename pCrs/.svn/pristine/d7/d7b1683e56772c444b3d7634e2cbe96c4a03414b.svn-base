...
if laid<>0 then 'Busco el registro para modificar
	...
	if not rs.eof then
		...
		cs="SELECT IdTipoHabitacion,CuantasHabis,NumAdultos,NumBebes,NumNinos1,NumNinos2," & chr(13) & chr(10)
		cs=cs & "Suplementos,IF(ISNULL(Traducciones.Traduccion),TipoHabitaNombres.Nombre,Traducciones.Traduccion) AS Tradu, TipoReserva.prepagoHab "& chr(13) & chr(10)
		cs=cs & "FROM " & precrs & "TipoReserva TipoReserva LEFT JOIN " & precrs & "TipoHabitaNombres TipoHabitaNombres "& chr(13) & chr(10)
	
		cs=cs & "LEFT JOIN " & precrs & "Traducciones Traducciones "& chr(13) & chr(10)
		cs=cs & "ON TipoHabitaNombres.id=Traducciones.IdReferencia "& chr(13) & chr(10)
		cs=cs & "AND Tabla='TipoHabitaNombres' AND Campo='Nombre' AND Idioma='" & lang & "' "& chr(13) & chr(10)

		cs=cs & "ON TipoReserva.IdTipoHabitacion=TipoHabitaNombres.Id "& chr(13) & chr(10)
		cs=cs & "WHERE IdReserva=" & laid& chr(13) & chr(10)
		'response.write(cs)
  ...
  <tr>
    <td class="tdTabla">
      <table align='center' width='100%' border='0' cellpadding="0" cellspacing="0" style="margin-top:10px;">
        <tr class="CabeTabla">
		  ...
		  <th align='left' class="colu_par"><%=objIdioma.getTraduccionHTML("i_prepago")%></th>
        </tr>
        <%if hayhabis then
	for h=0 to ubound(RegHabis,2)%>
		...
		%>
		  ...
		  <td align='left'> <%=RegHabis(HaPrepago,h)%> </td>
        </tr>
	...
	next 'Reghabis
		...
		<%if hayservis then%>
		<tr class="CabeTabla">
          <th align='left' class="colu_par" colspan="4"><%=objIdioma.getTraduccionHTML("i_servicios")%></th>
        </tr>
