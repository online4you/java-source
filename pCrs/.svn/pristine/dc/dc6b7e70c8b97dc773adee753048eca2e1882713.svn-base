...
<%if laspelas=0 then %>
	parent.$('#enlace_<%=idh%>_<%=th%>').css("display",'none');
	parent.$('#precio_<%=idh%>_<%=th%>').html('<b><%=objIdioma.getTraduccionHTML("i_nodisponibles")%></b>');
	parent.$('#precio_<%=idh%>_<%=th%>').attr("href",'#');
<%else%>
	parent.$('#enlace_<%=idh%>_<%=th%>').css("display",'block');
	parent.$('#enlace_<%=idh%>_<%=th%>').attr("href",'<%=enlacenuevo%>');
	parent.$('#precio_<%=idh%>_<%=th%>').html('<b><%=formatnumber(laspelas*elCambio,2) & sufijoMoneda%></b>');
	parent.$('#precio_<%=idh%>_<%=th%>').attr("href",'<%=enlacenuevo%>');
<%end if%>
