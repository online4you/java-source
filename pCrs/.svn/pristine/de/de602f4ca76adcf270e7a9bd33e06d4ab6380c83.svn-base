...
'Tabla de Hoteles
cs="SELECT CodigoEsta,Nombre FROM " & precrs & "Establecimientos "
if busco<>"" then
...
function cargaHotel(ese){
	top.document.getElementById('verFicha<%=idf%>').src="verUsuario.asp?id=<%=laid%>&bh="+ese+"&recarga=<%=recarga%>";
	cerrar();
}
...
<body class='laFicha'>
<form name='f1' action="<%=MiPag%>?id=<%=laid%>&idf=<%=idf%>" method="POST">
<div class="tituloFicha" id='tituloFicha'>
	<div class="nombreFicha">BUSCA HOTELES</div>
	<div class="laX" id='laX'></div>
	<img id='iconoForma' src="img/Mini.gif" border="0" width="21" height="17" class="Minimizar" alt="Minimizar" />
</div>
<div style="clear:both; margin-top:10px;">
<input name="buscoh" type="text" maxlength="50" style="width:200px; float:left; height:17px" onKeyPress="javascript:elIntro(event);" value="<%=busco%>">
