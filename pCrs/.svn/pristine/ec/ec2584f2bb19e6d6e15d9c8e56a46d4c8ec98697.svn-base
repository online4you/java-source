...
%><!--#include file="actuCategorias.asp"--><%

'Lista de registros
cs="SELECT Categorias.Id,Nombre,Idioma,Traduccion "
cs=cs & "FROM " & precrs & "Categorias Categorias LEFT JOIN " & precrs & "Traducciones Traducciones "
cs=cs & "ON Categorias.Id=Traducciones.IdReferencia "
cs=cs & "AND Tabla='Categorias' AND Campo='Nombre' "
rs.Open cs, base
hayCate=false
if not rs.eof then
	RegCate=rs.GetRows
...
if hayCate then
	...
	for t=0 to ubound(RegCate,2)
		if id_old<>RegCate(RcCodi,t) then 'crear linea
			nlista=nlista+1
			redim preserve RegLista(totalcampos,nlista)
			RegLista(RCodi,nlista)=RegCate(RcCodi,t)
			RegLista(RNombre,nlista)=RegCate(RcNombre,t)
		end if 'id_old
		id_old=RegCate(RcCodi,t)
		for idi=1 to ubound(ListaIdiomas)
			if ListaIdiomas(idi)=RegCate(RcLang,t) then 'este
				RegLista(TIdiomas(idi),nlista)=RegCate(RcTradu,t)
				exit for
			end if
		next 'idi
	next 't
...
function enBlanco(){
	top.creaFlotante("verCategorias.asp?id=0&recarga="+self.name,450,200,0,0);
}
function verFicha(id){
	top.creaFlotante("verCategorias.asp?id="+id+"&recarga="+self.name,450,200,0,0);
}
</script>
</head>
<body>
      ...
      <table border="0" align='left' cellpadding="0" cellspacing="0">
        <tr> 
          <td align="left" width="760"> <table border="0" cellpadding="0" cellspacing="0" style="margin-top:10px;" width="100%">
              <tr>
                <td align="right" colspan="<%=totalcampos+2%>"> <input type='button' class="boton145" style='cursor:pointer' onclick="javascript:enBlanco();" value='<%=objIdioma.getTraduccionHTML("i_nuevacategoria")%>'> 
              ...
              <tr>
                <td colspan="<%=totalcampos+2%>" align="left" class="tituloTabla"><%=Ucase(objIdioma.getTraduccionHTML("i_categorias"))%></td>
              </tr>
              <tr> 
                <th class="colu_par"></th>
                <th class="colu_par" align="center">ID</th>
                <th class="colu_par" align="left"><%=objIdioma.getTraduccionHTML("i_categoria")%></th>
