...
'Minimos y release
cs="SELECT Dia,CodigoHab,DiasMinimos,Release,SUM(Cuantas) as Disponibles "
...
cs=cs & ") GROUP BY CodigoHab,Dia,DiasMinimos,Release "
				...
				if isdate(midia) then
					...
					for hh=0 to ubound(RegLista,2)
						if RegLista(PDia,hh)=cdate(midia) AND RegLista(PHabi,hh)=RegHabis(HCodi,h) then 'este es el día
							haydia=true
							minimo=RegLista(PMinimo,hh)
							release=RegLista(PRelease,hh)
					...
					next 'hh
					if haydia then
						response.write "<td align='center'" & fondillo & " id='" & right("0"&day(midia),2) & "/" & right("0"&month(midia),2) & "/" & year(midia) & "_" & RegLista(PHabi,hh) & "' onclick='javascript:marcar(this);' class='celdilla'>" & valor & "</td>" & vbcrlf
					else
