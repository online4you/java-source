	...
	select case modo 
		case "borra" 'Borrar marcadas
			...
			if ubound(queborro)>=0 then
				...
				base.execute "DELETE FROM " & precrs & "Categorias WHERE " & cs
				controlRegistro("DELETE FROM " & precrs & "Categorias WHERE " & cs) 'guarda seguimiento
				
				'Borrar traducciones
				cs=replace(cs,"Id=","IdReferencia=")
				base.execute "DELETE FROM " & precrs & "Traducciones WHERE " & cs & " AND Tabla='Categorias'"
		...
		case "nuevo" 'Añadir
			...
			'Añadir
			cs="INSERT INTO " & precrs & "Categorias (Nombre,IdTipo) VALUES ('" & nombre & "'," & idtipo & ")"
			base.execute cs
			...
			for idi=1 to ubound(ListaIdiomas)
				if TIdiomas(idi)<>"" then 'crear registro
					...
					cs=cs & laulti & ",'" & listaIdiomas(idi) & "','Categorias','Nombre','"
					cs=cs & TIdiomas(idi) & "')"
					base.execute cs
		...
		case "actu"
			...
			'Actualiza Nombres
			cs="UPDATE " & precrs & "Categorias SET "
			cs=cs & "Nombre='" & nombre & "',"
			cs=cs & "IdTipo=" & idtipo & " "
			cs=cs & "WHERE Id=" & MiId
			base.execute cs
			...
			for idi=1 to ubound(ListaIdiomas)
				if TIdiomas(idi)<>"" then 'buscar si existe
					cs="SELECT Id FROM " & precrs & "Traducciones WHERE Tabla='Categorias' AND Campo='Nombre' AND "
					...
					if not rs.eof then
						...
						cs="INSERT INTO " & precrs & "Traducciones (IdReferencia,Idioma,Tabla,Campo,Traduccion) VALUES ("
						cs=cs & MiId & ",'" & listaIdiomas(idi) & "','Categorias','Nombre','"
						cs=cs & TIdiomas(idi) & "')"
						base.execute cs
					end if 'eof
					rs.close
					...
					cs="DELETE FROM " & precrs & "Traducciones WHERE Tabla='Categorias' AND Campo='Nombre' "
