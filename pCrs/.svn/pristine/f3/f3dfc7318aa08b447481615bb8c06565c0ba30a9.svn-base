...
if errormsg="" then 'sigo buscando
	...
	cs="SELECT ConsultaHoteles.CodigoEsta,Nombre,Categoria,Zona,ConsultaHoteles.Foto1,TipoAlojamiento,"
	cs=cs & "Texto1_" & lang & ",Estado"
	if TRegimen<>0 then
		cs=cs & ",RegimenHotel.Id, Regimen.Nombre_" & lang
	end if
	cs=cs & " FROM (ConsultaHoteles INNER JOIN PresentacionHotel "
	cs=cs & "ON ConsultaHoteles.CodigoEsta=PresentacionHotel.CodigoEsta) "
	...
	if errormsg="" then 'no hay error
		...
		cs="SELECT Id,Nombre_" & lang & " FROM TipoAlojamiento"
		rs.open cs,base
		haytipoa=false
		if not rs.eof then
			RegTipoA=rs.getrows
		...
		'Tabla categorias
		cs="SELECT Id,Nombre_" & lang & " FROM Categorias"
		rs.open cs,base
		haycate=false
		if not rs.eof then
			RegCate=rs.getrows
		...
		cs="SELECT Id,ParaCapNormal,ParaCapMax,Nombre_" & lang & ",CodigoEsta  FROM TipoHabitaNombres "
		...
		cs="SELECT CodigoTemp,CodigoEsta,FInicio,FFinal FROM Temporadas "
