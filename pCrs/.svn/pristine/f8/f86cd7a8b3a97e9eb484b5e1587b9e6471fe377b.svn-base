<%
		...
		PrecioHab(dia)=0
		PrecioPlazas(dia)=0
		cs="SELECT Preprebase,Preperhab,Nombre_" & lang & ",ParaCapNormal "
		cs=cs & "FROM TipoHabitaPrecios INNER JOIN TipoHabitaNombres "
		...
		if not rs.eof then 'Esa temporada
			...
			nombrehabi=rs("nombre_" & lang)
			response.write "HayHabi: " & nombrehabi & "<br/>"
		...
		if hayOferta then
			for o=0 to ubound(RegLOfertas,2)
				if clng(RegLOfertas(OfNoches,o))=0 or clng(RegLOfertas(OfNoches,o))<=clng(noches) then 'si tiene noches
					if RegLOfertas(OfAplicar,o)=ahabi or RegLOfertas(OfAplicar,o)=atodo then 'habitacion o todo
