...
If Request.ServerVariables("HTTPS")="off" Then
	...
	if request.QueryString<>"" then
		Response.Redirect strRedirURL & "?" & request.QueryString
	else
...
<div id='capaPrincipal'> 
  ...
  <div id='capaProceso'> 
    <%
		...
		url=url & query
		%>
    <iframe name="iframeContenido" id='iframeContenido' src="<%=url%>" onload='javascript:controlAlto();' frameborder="0"></iframe>
  </div>
  <!--capaProceso-->
  <div id='capaPie'> 
    <p>Kubik CRS ver 3.5&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&copy;&nbsp;2009 Emarketing 
      Planeta Web</p>
  </div>
  <!--capaPie-->
</div>
...
function controlAlto(){
	//alert(top.iframeContenido.document.getElementById('principalFrame').offsetHeight);
	miAlto=top.iframeContenido.document.getElementById('principalFrame').offsetHeight;
	$("#iframeContenido").height(miAlto+20);
}
</script>
