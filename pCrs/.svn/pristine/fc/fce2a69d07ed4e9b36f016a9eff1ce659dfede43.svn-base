...
rutaXML=server_path & "reservas/bookingFront/IdiomaXML/"
...
function enBlanco(){
	top.creaFlotante("idiomas/verTraduccion.asp?id=&recarga="+self.name,1000,330,0,0);
}
function verFicha(id){
	top.creaFlotante("idiomas/verTraduccion.asp?id="+id+"&recarga="+self.name,1000,330,0,0);
}
...
function buscaPalabra(){
	...
	if (busca!=""){
		top.creaFlotante("idiomas/buscadorPalabras.asp?busco="+busca+"&recarga="+self.name,600,400,0,0);
	}
          ...
          <form name='f1' action="<%=MiPag%>" method="POST">
            <table width='100%' border='0' cellpadding="0" cellspacing="0">
              ...
              <tr> 
                ...
                <th align='left' class="colu_par">
                  <%=objIdioma_bg.getTraduccionHTML("i_bulgaro")%>
                </th>
              </tr>
              <%if hayLista then
			...
			%>
              <tr> 
                ...
                <td align="left" class='<%=laColu(0)%>'> <a href='javascript:verFicha("<%=RegLista(LEti,r)%>");' title="<%=RegLista(LDetalle,r)%>"> 
                  <%=server.HTMLEncode(mid(RegLista(LTradu,r),1,40))%>
                  </a> </td>
                ...
                <td align="left" class='<%=laColu(0)%>' > <a href='javascript:verFicha("<%=RegLista(LEti,r)%>");' title="<%=RegLista(LDetalle,r)%>"> 
                  <%=mid(objIdioma_bg.getTraduccionHTML(RegLista(LEti,r)),1,40)%>
                  </a> </td>
              </tr>
              <%next
	end if%>
