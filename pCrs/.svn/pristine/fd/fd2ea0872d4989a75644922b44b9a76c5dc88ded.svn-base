...
cs=cs & " WHERE Estado<>" & noventa & buscoZona & buscoTipo
cs=cs & " ORDER BY Orde"
...
top.document.getElementById("listaHoteles").innerHTML="<a href=javascript:cambiaHotel(0,'<%=objIdioma.getTraduccionHTML("i_todos")%>')><%=objIdioma.getTraduccionHTML("i_todos")%></a>";
<%
if hayHoteles then
for h=0 to ubound(RegHoteles,2)
	...
	top.document.getElementById("listaHoteles").innerHTML=top.document.getElementById("listaHoteles").innerHTML+"<a href=javascript:cambiaHotel(<%=RegHoteles(HCodi,h)%>,'"+hotel+"')><%=RegHoteles(HNombre,h)%></a>";
<%next
end if%>
