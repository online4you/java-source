...
public class Disponibilidad extends MainAction {
	...
	private IBDLDisponibilidad dispo;
	private String zona;
	...
	public String executeHalconAction() throws Exception {
		...
		if (normalized || joomla){
			...
			dispo = this.getDisponibilidad(distribuciones,roomCount, fini, ffin, idSession, numPagina, idioma, zona,true,hotelid,sessionId);
			if(dispo!=null && dispo.getHoteles()!=null && dispo.getHoteles().size()>0){
				detalleDistribuciones=dispo.getHoteles().get(0).getContratos().get(0).getDistribuciones();
			}

			if(dispo!=null && dispo.getPeticiones()!=null && dispo.getPeticiones().size()>0){
				debug(dispo.getPeticiones().get(0));
				for (int i=0;i<dispo.getRespuestas().size();i++){
					debug(dispo.getRespuestas().get(i));
				}
			}
	...
	public boolean mostrarMasResultados(){
		String max=this.getConfigParam("maxHotelesEnPrimeraDispo");
		if(dispo.getHoteles().size()>Integer.parseInt(max)){
			return true;
		}else{
			if (todosLosResultados.equals("true")){
				return true;
	...
	public String recortaDescipcion(IBDLHotel hotel){
		String ret;
		int limite;
		try{
		...
		if (hotel.getDescripcion()!=null && hotel.getDescripcion().length()>limite){
			ret=hotel.getDescripcion().substring(0,limite);
		}else{
			ret=hotel.getDescripcion();
		}
		return ret;
	}
	...
	public IBDLDisponibilidad getDispo() {
		return dispo;
	}

	public void setDispo(IBDLDisponibilidad dispo) {
		this.dispo = dispo;
	}
