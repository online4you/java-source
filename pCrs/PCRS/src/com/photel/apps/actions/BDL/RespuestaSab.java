...
public class RespuestaSab extends MainAction {
	...
	public synchronized String executeHalconAction() throws Exception {
		...
		//String _idReserva = idReserva.split("-")[1];
		if (order.equals( idReserva.split("-")[1] ) && merchantCode.equals(idComercio)) {
			try {
				...
				if (idReserva.startsWith("B-")) {
					...
					//String idReserva =this.idReserva.split("-")[1];
					HibernateReservas res = this.service.getReservaHotelanFromLocata(this.idReserva);
