...
public abstract class MainAction extends ActionSupport implements SessionAware, ServletRequestAware, ServletResponseAware  {
	...
	protected final String[] BANNED_DOMAINS= {"yopmail.com","guerrillamail.com","thraml","maildrop.cc","nwytg.net","mailinator.com"};
		...
		for (int i=0;i<hotelBloqueado.getContratos().get(0).getDistribuciones().size();i++){
			...
			for (int p=0;p<precios.size();p++){
				if (fechaMasCercana==null){
					...
					if(fechaMasCercana.compareTo(precios.get(p).getCancelationFrom())>0){
						fechaMasCercana=precios.get(p).getCancelationFrom();
					}
				}
			}
	...
	public String getPaxesString(String adultos, String childs, String infants){
		String ret="";
		int adt;
		int chd;
		int inf;
		try {adt=Integer.parseInt(adultos);} catch (Exception e) {adt=0;}
		try {chd=Integer.parseInt(childs.split("-")[0]);} catch (Exception e) {chd=0;}
		try {inf=Integer.parseInt(infants);} catch (Exception e) {inf=0;}
		
		if (adt==1){
			ret+=adt+" "+this.getText("lang.gen.glo.Adulto");
		}else{
			ret+=adt+" "+this.getText("lang.gen.glo.Adultos");
		}
		if (chd==1){
			ret+=" "+chd+" "+this.getText("lang.gen.glo.nino");
		}else if (chd>1){
			ret+=" "+chd+" "+this.getText("lang.gen.glo.ninos");
		}
		if (inf==1){
			ret+=" "+inf+" "+this.getText("lang.gen.glo.bebe");
		}else if (inf>1){
			ret+=" "+inf+" "+this.getText("llang.gen.glo.bebes");
		}
		return ret;
	}
