...
public class HotelanFacade {
	...
	private List<Villa> villasThread;
	private IrqVillaAvailablePriceResponse dispoThread;
	...
	public IrqVillaConfirmReservationResponse getIrqVillaConfirmReservation(String reserveId, String cs) throws Exception {
		return client.getIrqVillaConfirmReservation(reserveId, cs);
	}
		...
		if (dispo!=null && dispo.getVilla()!=null && dispo.getVilla().size()>0){
			...
			for (Villa villa:dispo.getVilla()){
				...
				if (codes.containsKey(villa.getId())){
					...
					villa.setCodPortVill(codes.get(villa.getId()).get("codportvill_all"));
					...
					villa.setCodPortVill("P"+villa.getId());
	...
	public IrqVillaAvailablePriceResponse getVillaAvailableRich(String prepagoDefault,
		...
		if (hotelId==null ){
			dispo = client.getIrqVillaAvailablePrice(idioma, hotelId, destinationId,
				zoneId, arrival, nights, adults, children, infants);
		}else{
			if (hotelId.contains("##")){
				String[] ids = hotelId.split("##");
				if (ids.length>3){
					dispo = client.getIrqVillaAvailablePrice(idioma, null, destinationId,
							zoneId, arrival, nights, adults, children, infants);
				}else{
					String idPrimero=null;
					villasThread=new ArrayList<Villa>();
					List<ThreadBase> hilos=new ArrayList<ThreadBase>();
					for (String id:ids){
						String idHotel = id.substring(1,id.length());
						if(idHotel!=null && !idHotel.equals("")){
						idPrimero=idPrimero==null?idHotel:idPrimero;
						Class[] tipoParams={String.class, 
								String.class, 
								String.class, 
								String.class, 
								Calendar.class, 
						...
						hilos.add(ThreadManager.start("HILO_threadDispoVillas",this, "threadDispoVillas",tipoParams,params,45000));
						}
					}
					
					
					ThreadManager.esperarTerminacionHilos(hilos);
					if (dispoThread!=null){
						dispo=dispoThread;
						dispo.setVilla(villasThread);
					}else{
						dispo = client.getIrqVillaAvailablePrice(idioma, idPrimero, destinationId,
								zoneId, arrival, nights, adults, children, infants);
					}
				}
			}else{
				String idHotel = hotelId.substring(1,hotelId.length());
				dispo = client.getIrqVillaAvailablePrice(idioma, idHotel, destinationId,
						zoneId, arrival, nights, adults, children, infants);
			}
		}
	...
	public void threadDispoVillas(String idioma, 
			String idHotel,
			...
			Integer children,
			Integer infants) throws Exception{
		IrqVillaAvailablePriceResponse dispo = client.getIrqVillaAvailablePrice(idioma, idHotel, destinationId,
				zoneId, arrival, nights, adults, children, infants);
		if (dispo.getVilla()!=null && dispo.getVilla().size()>0){
			villasThread.addAll(dispo.getVilla());
		}
		if (dispoThread==null){
			dispoThread=dispo;
		}
	...
	public HibernateReservas getReservaFromLocata(String locata) {
		return ddbb.getReservaFromLocata(locata);
	}
