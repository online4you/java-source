...
import com.photel.commonServices.util.HTMLUtil;
...
public class PCRSModel extends UnicastRemoteObject implements java.io.Serializable {
	...
	private Hashtable<String,Object> getAtthotelan(String idi, String numCuenta, String locata)throws Exception{
		String ret="";
		HibernateReservas reser = hotelan.getReservaFromLocata(locata);
		String hotelStr=reser.getHlHotel();
		Villa hotel = (Villa) XMLStreamUtil.xmlLoadObject(hotelStr);
		...
		GregorianCalendar cal=new GregorianCalendar();
		Date fecha = reser.getHlEntrada();
		cal.setTime(fecha);
		att.put("fechaEntrada", formatDate(cal));
		cal=new GregorianCalendar();
		fecha = reser.getHlSalida();
		cal.setTime(fecha);
		att.put("fechaSalida", formatDate(cal));
		...
		Double pagado = reser.getHlImportePagado();
		Double total = reser.getHlImporteTotal();
		Double resto = total-pagado;
		Double impGastos=reser.getHlGastos();
		...
		att.put("PortVillLocata", reser.getHlLocata().split("-")[1]);
		...
		String antesDe=(String) att.get("lang_gen_crs_i_aPagarAntesDe");
		Date gastos = reser.getHlFechaGastos();
		cal.setTime(gastos);
		String diasAntes=getGenConfig().get("diasDeAntelacionAcobrarTodoTransfer");
		if (reser.getHlRadioTipoPago().equals("1")){
			diasAntes=getGenConfig().get("diasDeAntelacionAcobrarTodoTPV");
			att.put("tipoTPV", reser.getHlRadioTipoPago());
		}else if(reser.getHlRadioTipoPago().equals("2")){
			diasAntes=getGenConfig().get("diasDeAntelacionAcobrarTodoTransfer");
			att.put("tipoTransfer", reser.getHlRadioTipoPago());
			att.put("nombreBanco", textos.get("lang.gen.glo.nombreBanco"));
			att.put("nombreCuenta", textos.get("lang.gen.glo."+numCuenta+"nombreCuenta"));
			att.put("numerobic", textos.get("lang.gen.glo."+numCuenta+"numerobic"));
		}
		...
		format.format(reser.getHlFechaGastos());
		cal2=format.getCalendar();
		String fechaGastos=formatDate(cal2);
		antesDe+=" "+ fechaGastos;
		att.put("aPagarAntesDe",antesDe);
		att.put("fechaGastos", fechaGastos);
		
		
		if(reser.getHlCheckContacto().equals("1")){
			att.put("contacto", reser.getHlCheckContacto());
		}
		if(reser.getHlCheckAceptoCondidiones().equals("1")){
			att.put("aceptoCondiciones", reser.getHlCheckAceptoCondidiones());
		}
		if(reser.getHlCheckRecibirOfertas().equals("1")){
			att.put("recibirOfertas", reser.getHlCheckRecibirOfertas());
		}
		if(reser.getHlCheckFactura().equals("1")){
			att.put("factura", reser.getHlCheckFactura());
			if (reser.getHlRadioTipoPago().equals("1")){
				cal.setTime(reser.getHlTimcre());
				String fact=+cal.get(Calendar.YEAR)+"-"+getGenConfig().get("prefijoFacturaPortVill")+"-"+reser.getHlFacturaNumero();
				att.put("numfactura", fact);
			}else{
				att.put("numfactura", "-");
			}
	...
	public String sendVoucherHotelan(String idi, String numCuenta, String locata, String cc,String cco, String template,boolean cancelacion)throws Exception{
		Hashtable<String, Object> att = this.getAtthotelan(idi, numCuenta, locata);
		String asunto=(String) att.get("lang_gen_crs_i_confireserva");
		if (cancelacion){
			asunto=(String) att.get("lang_gen_glo_textoCancelacionReserva");
		}
		...
		HibernateReservas reser=(HibernateReservas) att.get("reserva");
		if(reser.getHlLocata().split("-").length==2){
			asunto+=" "+reser.getHlLocata()+"-"+reser.getId()+" "+reser.getHlNombreHotel();
		}else{
			asunto+=" "+reser.getHlLocata()+" "+reser.getHlNombreHotel();
		}
		String to=reser.getHlEmail();
		//to="guillempugros@gmail.com";
		String from=this.getGenConfig().get("voucherFROM");
		String result = sendTemplate(null,template,"any", att,asunto, to,cc,cco, from);
		boolean ok=result!=null && !result.equals("")?true:false;
		Integer IdReserva=reser.getId();
		if(!ok){
			asunto="Error en el envio a "+to+" (" + asunto +")";
			result = sendTemplate(null,template,"any", att,asunto, from,cc,cco, from);
			HibernateReservasMails mail = hotelan.getReservaMail(0);
			mail.setHlAsunto(asunto);
			mail.setHlCc(cc);
			mail.setHlCco(cco);
			mail.setHlFrom(from);
			mail.setHlIdReserva(IdReserva);
			mail.setHlMail(result);
			mail.setHlTo(from);
			hotelan.setReservaMails(mail);
		}else{
			HibernateReservasMails mail = hotelan.getReservaMail(0);
			mail.setHlAsunto(asunto);
			mail.setHlCc(cc);
	...
	public String sendVoucherHotelan(String idi, String numCuenta, String locata)throws Exception{
		String cco=this.getGenConfig().get("voucherCCO");
		String cc=this.getGenConfig().get("voucherCC");
		//cco="guillempugros@gmail.com";
		//cc="guillempugros@gmail.com";
		String ret=sendVoucherHotelan(idi, numCuenta, locata, cc,cco,"voucherTPVPortVill",false);
		return ret;
	}
	...
	private String getIdDisponibilidadHotelan(String idioma,
		...
		String id="idioma="+idioma;
		id+="hotelId"+hotelId;
		id+="destinationId"+destinationId;
		id+="zoneId"+zoneId;
		id+="_entrada="+arrival.get(Calendar.YEAR)+arrival.get(Calendar.MONTH)+arrival.get(Calendar.DAY_OF_MONTH);
		id+="nights"+nights;
		id+="adults"+adults;
	...
	private void setCacheHotelan(IrqVillaAvailablePriceResponse dispo, String idioma,
			String hotelId, String destinationId, String zoneId,
			Calendar arrival, Integer nights, Integer adults, Integer children,
			Integer infants,String sessionId) throws SQLException, Exception {
		
				String setCache = modelGen.getGenConfig().get("cachearDisponibilidades");
				if (setCache!=null && setCache.equalsIgnoreCase("true")){
					String id=getIdDisponibilidadHotelan(idioma, hotelId, destinationId, zoneId, arrival, nights, adults, children, infants, sessionId);
					id = "DISPO_" + id;
					GregorianCalendar cal = new GregorianCalendar();
					SerializableHelper help = new SerializableHelper();
	...
	private IrqVillaAvailablePriceResponse getCacheHotelan(String idioma,
			String hotelId, String destinationId, String zoneId,
			Calendar arrival, Integer nights, Integer adults, Integer children,
			Integer infants,String sessionId) throws SQLException, Exception {
				
			IrqVillaAvailablePriceResponse dispo = null;
				String setCache = modelGen.getGenConfig().get("cachearDisponibilidades");
				if (setCache!=null && setCache.equalsIgnoreCase("true")){
					String minutosCache = modelGen.getGenConfig().get("tiempoMinutosCaheDispo");
					...
					if (minutosCache != null) {
						...
						String id=this.getIdDisponibilidadHotelan(idioma, hotelId, destinationId, zoneId, arrival, nights, adults, children, infants, sessionId);
						...
						if (cache != null) {
								Hashtable<String, Object> content = (Hashtable<String, Object>) help.unserialize(cache.getTrdDetalle());
								dispo = (IrqVillaAvailablePriceResponse) content.get("dispo");
						}
					}
				}
				return dispo;
	...
	public IrqVillaAvailablePriceResponse getVillaAvailableRich(String idioma,
			String hotelId, String destinationId, String zoneId,
			Calendar arrival, Integer nights, Integer adults, Integer children,
			Integer infants, String sessionId) throws Exception {
		
		IrqVillaAvailablePriceResponse ret=null;
		
		if(sessionId!=null){
			ret=this.getCacheHotelan(idioma, hotelId, destinationId, zoneId, arrival, nights, adults, children, infants, sessionId);
		}
		if (ret==null){
			String prepagoDefault=modelGen.getGenConfig().get("portVillDefaultComision"); 
			ret=hotelan.getVillaAvailableRich(prepagoDefault,idioma, hotelId, destinationId,zoneId, arrival, nights, adults, children, infants);
			if (ret!=null && ret.getVilla()!=null && ret.getVilla().size()>0 && sessionId!=null){
				this.setCacheHotelan(ret, idioma, hotelId, destinationId, zoneId, arrival, nights, adults, children, infants, sessionId);
			}
		}
		return ret;
	}
	...
	public Villa getVilla(Villa vill, String lang) {
		return hotelan.getVilla(vill, lang);
	}
	...
	public IrqVillaPreReservationResponse getIrqVillaPreReservation(
			...
			throws Exception {
		return hotelan.getIrqVillaPreReservation(idioma, hotelId, arrival,
	...
	public IrqVillaConfirmReservationResponse getIrqVillaConfirmReservation(String reserveId, String cs) throws Exception {
		return hotelan.getIrqVillaConfirmReservation(reserveId, cs);
	}
	public HibernateReservas getReservaHotelan(int idReserva) {
		return hotelan.getReserva(idReserva);
	}
	public HibernateReservas getReservaHotelanFromLocata(String locata) {
		return hotelan.getReservaFromLocata(locata);
	}
	public HibernateReservas setReservaHotelan(HibernateReservas res) {
		return hotelan.setReserva(res);
	}
	public HibernateReservasMails getReservaMailHotelan(int idReservaMail) {
		return hotelan.getReservaMail(idReservaMail);
	}
	public HibernateReservasMails setReservaMailsHotelan(HibernateReservasMails res) {
		return hotelan.setReservaMails(res);
	}
	public HibernateReservas getUltimaFacturaHotelan() {
		return hotelan.getUltimaFactura();
	}
	public void updateHotelan() throws Exception {
		Calendar ini=new GregorianCalendar();
		hotelan.update();
	
		String mail="guillempuigros@gmail.com";
		//String mail=this.getGenConfig().get("voucherFROM");
		String from=this.getGenConfig().get("voucherFROM");
		Hashtable<String, Object> att=new Hashtable<String, Object>();
		att.put("ini", SystemHelper.getCalendarTo_ddMMYYYYHHmmss(ini));
		att.put("fin", SystemHelper.getCalendarTo_ddMMYYYYHHmmss(new GregorianCalendar()));
		att.put("elapsed", SystemHelper.getTimeElapsedMinutes(ini));
		
		this.sendTemplate(null, "actualizacionPortVill", "any", att, "Actualización PortVill", mail, null, null, from);
	}
	public void updateHotelanThread() throws Exception {
	
	Class[] tipoParams={};
	Object[] params={};
	
	ThreadManager.start("HILO_updateHotelan",this, "updateHotelan",tipoParams,params,4000);
	
	}
	
	public String decodeHTML(String html){
		if (html!=null){
			html=HTMLUtil.decode(html);
		}
		return html;
	}
	public String encodeHTML(String html){
		if (html!=null){
			html=HTMLUtil.encode(html);
		}
		return html;
	}
}
